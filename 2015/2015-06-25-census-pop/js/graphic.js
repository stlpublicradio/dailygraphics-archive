...
//            return d['amt'].length > 0;
    ...
    var yScale = d3.scale.linear()
        .domain([ -2, d3.max(d3.entries(formattedData), function(c) {
                return d3.max(c['value'], function(v) {
                    var n = v[valueColumn];
                    return Math.ceil(n / 1) * 1; // round to next 5
                });
            })
        ])
        .range([ chartHeight, 0 ]);
    ...
    var colorScale = d3.scale.ordinal()
        ...
        .range([ colors['ltgreen'], colors['green'], colors['ltblue'], colors['blue'], colors['red'], colors['maroon'], colors['orange'], colors['brown'], colors['tan'] ]);
