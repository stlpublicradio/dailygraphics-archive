...
function fmtCurrency(d) {
	var fmtMoney = d3.format('.3s');
	return "$" + fmtMoney(d).replace('M', ' million').replace('G', ' billion')
    ...
    if (Modernizr.svg) {
        ...
        d3.csv(GRAPHIC_DATA_URL, function(error, data) {
            ...
            color = d3.scale.ordinal()
                .range([colors['tan'], colors['blue'], colors['red']])
                .domain(d3.keys(graphicData[0]).filter(function(key) { return key !== 'Department'; }));
