    ...
    if (Modernizr.svg) {
        ...
        d3.csv(GRAPHIC_DATA_URL, function(error, data) {
            ...
            color = d3.scale.ordinal()
                .range([colors['ltblue'], colors['dkblue']])
                .domain(d3.keys(graphicData[0]).filter(function(key) { return key !== 'Group'; }));
		
            colorMo = d3.scale.ordinal()
                .range([colors['red'], colors['maroon']])
                .domain(d3.keys(graphicData[0]).filter(function(key) { return key !== 'Group'; }));
...
var drawGraph = function(graphicWidth) {
    ...
    var barGroup = svg.selectAll('.bars')
        ...
        .enter().append('g')
	        .attr('class', function(d) {
	            return 'g bars s-' + classify(d['key']);
	        })
    ...
    barGroup.selectAll('rect')
        ...
        .enter().append('rect')
		    ...
		    // console.log(d)
		    // if (d['key'] !== 'Mo.') return color(d['label']); else return colorMo(d['label']);
		    //             })
            .attr('class', function(d) {
                return 'y-' + d['label'];
            });
