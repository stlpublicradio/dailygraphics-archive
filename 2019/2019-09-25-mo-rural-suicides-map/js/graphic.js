...
var colors = {
    'brown': '#6b6256','tan': '#a5a585','ltltlt' : '#ECF2F1', 'ltltgreen': '#98B8B6', 'ltgreen': '#6E9B99','green': '#457F7C','dkgreen': '#2C6563','ltblue': '#55b7d9','blue': '#358fb3','dkblue': '#006c8e','yellow': '#f1bb4f','orange': '#f6883e','tangerine': '#e8604d','red': '#cc203b','pink': '#c72068','maroon': '#8c1b52','purple': '#571751'
};
...
function render(width) {
		...
		var northEast = L.latLng(42.92, -85.89), 						
		    southWest = L.latLng(36.0, -96.27),
			bounds = L.latLngBounds(southWest, northEast);

		if (width < 500) {
			var map = L.map('map', {scrollWheelZoom: false, attribution: ''}).setView(bounds.getCenter(), 5).setMaxBounds(bounds);
		} else {
			var map = L.map('map', {scrollWheelZoom: false, attribution: ''}).setView(bounds.getCenter(), 6).setMaxBounds(bounds);
		}
	    ...
	    var svg = d3.select(map.getPanes().overlayPane).append("svg"),
			...
			// update filename, collection name
			 ...
			 return d < 1.00 ? colors.ltltlt :
			 d < 1.50 ? colors.ltltgreen :
			 d < 2.00 ? colors.ltgreen :
			 d < 2.50 ? colors.green :
			 colors.dkgreen;
				 ...
				 function style(feature) {
						 return {
							 // update property
					         fillColor: getColor(feature.properties.data_smr),
					         weight: 1,
					         opacity: 1,
					         color: colors.yellow,
					         dashArray: '3',
				 ...
				 info.update = function (props) {
				     this._div.innerHTML = '<h4>Suicide Risk</h4><br>' 
					 
					//  example ternary logic for infobox
					 +  (
						 props ?
						 "<strong>" + props.data_county + '</strong> has a standardized mortality ratio of <strong>' + Math.round(props.data_smr * 100) / 100 + '</strong>'
						 : 'Hover over a county');
				 };
				 ...
				 legend.onAdd = function (map) {
					 ...
					 // Legend with numbers
					 grades = [ 0.50, 1.00, 1.50, 2.00, 2.50],
