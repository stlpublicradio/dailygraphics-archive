...
function render(width) {
		...
		var northEast = L.latLng(38.524806, -89.945374), 												 			
		    southWest = L.latLng(38.501514, -90.018392),
			bounds = L.latLngBounds(southWest, northEast);

		var map = L.map('map', {zoomControl: false, attribution: ''}).setView(bounds.getCenter(), 16).setMaxBounds(bounds);
	    ...
	    var svg = d3.select(map.getPanes().overlayPane).append("svg"),
			...
			var myIcon = L.divIcon({className: 'my-div-icon', html: 'Belleville Square'});

			L.marker([38.514178241933, -89.98421788215637], {icon: myIcon}).addTo(map);
