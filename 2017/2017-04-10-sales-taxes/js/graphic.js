    ...
    _.each(['shadow', 'value'], function(cls) {
        chartElement.append('g')
            ...
            .enter().append('text')
                ...
                .text(function(d) {
                    if (d[valueColumn] == d[maxColumn]) {
                        return (d[valueColumn] * 100).toFixed(3) + '%';
                    }
                    else {
                        return (d[valueColumn] * 100).toFixed(3) + '% - ' + (d[maxColumn] * 100).toFixed(3) + '%';
                    }
                });
    });
...
// }, 2000);
    ...
    function change() {
        ...
        var newData = config.data.sort(this.checked ? function(a,b) { return b.amt - a.amt } : function(a,b) { return a.id - b.id });

        config.data = newData;

        renderDotChart(config);
