...
var COUNTY_LABELS_HIDDEN = [
    'Jefferson','Miller','Benton','Henry','Johnson','Livingston','Grundy','Mercer','St. Louis','St. Charles','Franklin','Gasconade','Osage','Cole','Moniteau','Morgan','Pettis','Cooper','Boone','Callaway','Montgomery','Warren','Lincoln','Montgomery','Audrain','Monroe','Howard','Saline','Lafayette','Carroll','Putnam','Sullivan']
...
var renderLocatorMap = function(config) {
    ...
    chartElement.append('g')
        ...
        .selectAll('.label')
            .data(mapData['countries']['features'])
        .enter().append('text')
            .filter(function(d) { return d['properties']['state'] == '29' })
            .attr('class', function(d) {
        ...
        COUNTY_LABELS_HIDDEN.forEach(function(label) {
            console.log(label)
            classed = '.label.' + classify(label)
            d3.select(classed).remove();
        });
