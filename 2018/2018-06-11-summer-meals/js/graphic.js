...
function render(width) {
	...
	var northEast = L.latLng(41.6136, -88.0995),
		    southWest = L.latLng(34.9957, -96.7747),
	    ...
	    var svg = d3.select(map.getPanes().overlayPane).append("svg"),
			...
			// update filename, collection name
			 ...
			 d3.json("summer_meals.topojson", function(error, geodata) {
				 
				 var regions = topojson.feature(geodata, geodata.objects.county_coverage);
				 var geojson;
			 ...
			 return d < .05 ? colors.red :
			 		d < .10 ? colors.tangerine :
			 		d < .15 ? colors.orange :
				 	d < .20 ? colors.yellow :
			 		d < .25 ? colors.green :
				 	colors.dkgreen;
				 ...
				 function style(feature) {
						 return {
							 // update property
					         fillColor: getColor(feature.properties.county_cov),
					         weight: 1,
					         opacity: 1,
					         color: colors.yellow,
					         dashArray: '3',
				 ...
				 info.update = function (props) {
				     this._div.innerHTML = '<h4>2017 coverage</h4>' 
					 
					 // example ternary logic for infobox
					 +  (
						 props ?
						props.NAME + " served meals meeting " + (props.county_cov*100).toFixed(1) + '% of the demand in 2017.'
						 : 'Hover over a county');
				 };
				 ...
				 legend.onAdd = function (map) {
					 ...
					 // Legend with numbers
					 grades = [.04, .09, .14, .19, .24],
					 text = ['< 5%','5-10%','10-15%','15-20%','20-25%','>25%']
					 labels = [];
