...
var COUNTRY_LABEL_ADJUSTMENTS = {
    'Jefferson': { 'dy': -50, 'dx': -20 },
    'Washington': { 'dy': 20, 'dx': -50 },
}
...
var renderLocatorMap = function(config) {
     ...
     * Setup
     */
    var aspectWidth = 1.55;
    var aspectHeight = 2;

    var bbox = config['data']['bbox'];
    var defaultScale = 28000;
    var cityDotRadius = 3;
    ...
    chartElement.select('.st-louis.label')
        .remove()
    chartElement.select('.monroe.label')
        .remove()    
    chartElement.select('.franklin.label')
    .remove()    
    chartElement.select('.ste-genevieve.label')
    .remove()    
    ...
    chartElement.append('g')
            ...
            .attr('transform', function(d) {
                        if (d['slug'] == 'big') {
                            return 'translate(' + xScale(d['x']) + ',' + yScale(d['y']) + ') rotate(-48 0 0)';
                        }
                        else if (d['slug'] == 'mississippi') {
                            return 'translate(' + xScale(d['x']) + ',' + yScale(d['y']) + ') rotate(-90 0 0)';
                        }
