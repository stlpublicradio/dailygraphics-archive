    ...
    d3.json("mocountieskc.json", function (error, geodata) {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            for (var k = 0; k < d3.entries(config.candidates).length; k++) {
                ...
                if (typeof results[i][party] == 'undefined') {
                    results[i][party] = 0
                }

                if (party == "rep" || party == "dem") {
                    stuff[party] = +results[i][party]
                } else {
                        other += +results[i][party]
            ...
            for (var j = 0; j < regions.features.length; j++) {
                ...
                if (mapCountyFP == dataCountyFP) {
                    regions.features[j].properties.rep = stuff["rep"];
                    regions.features[j].properties.dem = stuff["dem"];
                    regions.features[j].properties.other = stuff["other"];
                    regions.features[j].properties.total = total;
                    regions.features[j].properties.r_pct = stuff["rep"] / total;
                    regions.features[j].properties.d_pct = stuff["dem"] / total;
                    regions.features[j].properties.o_pct = stuff["other"] / total;
                    regions.features[j].properties.r_lead = (stuff["rep"] / total) - (stuff["dem"] / total);
                }
            }
        }
        ...
        var color = d3.scale.linear()
            ...
            .range([d3.rgb("#006c8e"), d3.rgb("#fff"), d3.rgb("#cc203b")])
        ...
        var MakeText = function (row) {
            ...
            for (var i = 0; i < d3.entries(config.candidates).length; i++) {
                ...
                data[d3.entries(config.candidates)[i]["key"]] = {
                    "candidates": d3.entries(config.candidates)[i]["value"],
                    "pct": cand_pct,
                    "total": +results[row][d3.entries(config.candidates)[i]["key"]]
                };
            }
            ...
            updateSelection
                ...
                .html(function (d) {
                    return d["value"]["candidates"] + ": <span class='pct'>" + fmtPct(d["value"]["pct"]) + "</span><br/><span class='votes'>(" + fmtComma(d["value"]["total"]) + ")</span>";
                })
        ...
        chartElement.selectAll(".county")
            ...
            .enter().append("path")
            .attr("class", function (d) {
                return "county county-" + d.properties.COUNTYFP;
            })
            .attr("stroke", "#333")
        ...
        var xScale = d3.scale.linear()
            .domain(getLeadCap(regions.features))
            .range([0, (chartWidth - 40) / 2, chartWidth - 40])
        ...
        distribution.selectAll("line")
            ...
            .enter().append("line")
            .attr("class", function (d) {
                return "county-line county-" + d.properties.COUNTYFP;
            })
            .attr("x1", function (d) {
                if (d.properties.r_lead) {
            ...
            .attr("stroke", function (d) {
                ...
                return d.properties.r_lead > 0 ? '#cc203b' : '#006c8e'
                // return color(d.properties.r_lead * 100)
            })
            .attr("stroke-width", isMobile ? 3 : 3)
            .on("mouseover", function (d) {
                for (var i = 0; i < results.length; i++) {
                    if (results[i]["name"] == d.properties.NAMELSAD) {
                        MakeText(i)
        ...
        distribution.selectAll(".county-triangle")
            ...
            .attr("class", function (d) {
                return "hidden county-triangle county-" + d.properties.COUNTYFP;
            })
        ...
        yLegendText.append("text")
            .text("Number of votes in county")
