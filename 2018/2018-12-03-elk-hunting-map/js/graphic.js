...
var render = function(containerWidth) {
    ...
    renderLocatorMap({
        ...
        data: geoData,
        primaryCountry: ['Reynolds', 'Shannon','Carter'],
        pixelOffset: [0, 0]
    });
...
var renderLocatorMap = function(config) {
    ...
    config['primaryCountry'].forEach(function(d) {
        var primaryCountryClass = classify(d);
    ...
    var highlight = ['Reynolds','Shannon','Carter']

    chartElement.append('g')
        .attr('class', 'country-labels')
        .selectAll('.label')
            .data(mapData['countries']['features'])
        .enter().append('text').filter(function(d) { return highlight.indexOf(d['properties']['country']) > -1; })
            .attr('class', function(d) {
