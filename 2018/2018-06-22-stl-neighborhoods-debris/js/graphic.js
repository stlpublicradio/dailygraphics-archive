...
function render(width) {
		...
		var northEast = L.latLng(38.80714,-90.033967),
		    southWest = L.latLng(38.50187,-90.441206),
	    ...
	    var svg = d3.select(map.getPanes().overlayPane).append("svg"),
			...
			// update filename, collection name
			 ...
			 d3.json("neighborhoods.topojson", function(error, geodata) {
				 ...
				 var regions = topojson.feature(geodata, geodata.objects.joined);
				 var geojson;
			 ...
			 return d > 500 ? colors.red :
			 		d > 400 ? colors.tangerine :
			 		d > 300 ? colors.orange :
				 	d > 200 ? colors.yellow :
			 		d > 100 ? colors.green :
				 	colors.dkgreen;
				 ...
				 info.update = function (props) {
					if (props) {
						if (props.incidents == null) {
							var incidents = '0 reports'
						}
						else {
							if (props.incidents == 1) {
								var incidents = '1 report'
							}
							else {
								var incidents = props.incidents + ' reports'
							}
						}
				     ...
				     this._div.innerHTML = '<h4>2017 CSB debris reports</h4><br/>' 
					 
					 // example ternary logic for infobox
					 +  (
						 props ?
						'<strong>' + props.neighborho + '</strong> had ' + incidents + ' of debris or illegal dumping in 2017.' 
				 ...
				 legend.onAdd = function (map) {
					 ...
					 text = ['< 100','100-200','200-300','300-400','400-500','> 500']
					 labels = [];
