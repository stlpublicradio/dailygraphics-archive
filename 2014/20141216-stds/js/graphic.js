...
var GRAPHIC_DATA_URL_2 = 'data2.csv';
var GRAPHIC_DEFAULT_WIDTH = 580;
var MOBILE_THRESHOLD = 540;

var colors = {
    'red1': '#cc203b','yellow1': '#f1bb4f','blue1': '#358fb3'
};
...
var onWindowLoaded = function() {
    if (Modernizr.svg) {
        ...
        d3.csv(GRAPHIC_DATA_URL_2, function(error, data) {
            graphicData2 = data;
...
var drawGraph = function(graphicWidth) {
    ...
    var color = d3.scale.ordinal()
        .range([ colors['red1'], colors['yellow1'] ]);
    var graph1 = d3.select('#graphic1');
    var graph2 = d3.select('#graphic2');
    var margin = { 
    	top: 5, 
    	right: 15, 
    	bottom: 30, 
...
var drawGraph2 = function(graphicWidth) {
    ...
    // assign a color to each line
    color.domain(d3.keys(graphicData2[0]).filter(function(key) { 
        return key !== 'date';
    }));
    ...
    for (var column in graphicData2[0]) {
        if (column == 'date') continue;
        syphillisData[column] = graphicData2.map(function(d) {
            return { 'date': d['date'], 'amt': d[column] };
// filter out empty data. uncomment this if you have inconsistent data.
//        }).filter(function(d) {
//            return d['amt'].length > 0;
    ...
    // set the data domain
    x.domain(d3.extent(graphicData2, function(d) { 
        return d['date'];
    }));

    y.domain([ 0, d3.max(d3.entries(syphillisData), function(c) { 
            return d3.max(c['value'], function(v) { 
                var n = v['amt'];
                return Math.ceil(n/5) * 5; // round to next 5
            }); 
